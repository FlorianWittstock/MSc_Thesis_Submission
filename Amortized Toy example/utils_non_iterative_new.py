...
def experiment_name(args):
    ...
    if args.problem_name in ['tail_integral_1d', 'tail_integral_5d']:
        return f'{args.problem_name}_{args.q_ratio}'
    else:
...
def get_model(args):
    ...
    """
    if args.problem_name in ['tail_integral_1d', 'tail_integral_5d']:
        return importlib.import_module(f'model_non_iterative_new')
    else:
        raise Exception(f'Unknown problem_name: {args.problem_name}.')
...
def sacred_main_helper(train_func, args, _run):
    ...
    repo = git.Repo('../..')   #angeblich kann ich die und die nächste zeile löschen
...
def generate_samples_for_evaluation(number_of_samples_total, number_of_samples_gpu_capacity, ys_thetas, model,
    ...
    for y_theta in tqdm.tqdm(dict_of_lists_to_list_of_dicts(ys_thetas)):
        ...
        for _ in tqdm.tqdm(range(int(math.ceil(number_of_samples_total/number_of_samples_gpu_capacity))),
                           disable=False):
            # returns a dict
            data_dict_q_ratio = model.samples_for_evaluation(
                min(number_of_samples_left, number_of_samples_gpu_capacity),
                y_theta=y_theta, device=device)

            data_dicts_q_ratio_temp.append(data_dict_q_ratio)

            number_of_samples_left -= number_of_samples_gpu_capacity

        data_dicts_q_ratio.append({k: torch.cat(tuple(d[k] for d in data_dicts_q_ratio_temp), dim=0) for k in data_dicts_q_ratio_temp[0].keys()})
    print("Debug: data_dict_q_ratio keys:", data_dict_q_ratio.keys())
    print("Debug: data_dict_q_ratio content:", data_dict_q_ratio)
    #print("Debug: data_dict_q_ratio_temp keys:", data_dict_q_ratio.keys())
    #print("Debug: data_dict_q_ratio_temp content:", data_dict_q_ratio)
    # lists of dicts
    #data_dict_q_ratio = map(
        #lambda data_dicts: {k: torch.stack(tuple(d[k] for d in data_dicts), dim=0) for k in data_dicts[0].keys()},
        #(data_dicts_q_ratio))
    
    #data_dict_q_ratio = {k: torch.stack(tuple(data_dicts_q_ratio[k]), dim=0) for k in data_dicts_q_ratio[0].keys()}
    data_dict_q_ratio = {k: torch.stack(tuple(d[k] for d in data_dicts_q_ratio), dim=0) for k in data_dicts_q_ratio[0].keys()}

    return data_dict_q_ratio
